...
public class ReelCounterLogic : BaseNetLogic
{
    ...
    private void ComponentsCounter()
    {
        if (Project.Current.GetVariable("Model/Cycle/Start").Value)
        {
            ...
            if (rnd.Next(0, 3) == 1 && reelCounter1.Value > 0)
            {
                --reelCounter1.Value;
            }
            Thread.Sleep(1);
            if (rnd.Next(0, 3) == 1 && reelCounter2.Value > 0)
            {
                --reelCounter2.Value;
            }
            Thread.Sleep(1);
            if (rnd.Next(0, 3) == 1 && reelCounter3.Value > 0)
            {
                --reelCounter3.Value;
            }
            Thread.Sleep(1);
            if (rnd.Next(0, 3) == 1 && reelCounter4.Value > 0)
            {
                --reelCounter4.Value;
            }
            Thread.Sleep(1);
            ...
            if (reelCounter1.Value == 0 ||
                reelCounter2.Value == 0 ||
                reelCounter3.Value == 0 ||
                reelCounter4.Value == 0)
            {
                Project.Current.GetVariable("Model/Cycle/Start").Value = false;
            }
        }
    }
}
